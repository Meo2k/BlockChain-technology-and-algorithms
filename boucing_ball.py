...
def draw_arc(window, center, radius, start_angle, end_angle):
    p1 = center + (radius + 100) * \
        np.array([math.cos(start_angle), math.sin(start_angle)])
    p2 = center + (radius + 100) * \
        np.array([math.cos(end_angle), math.sin(end_angle)])
    pygame.draw.polygon(window, BALL_RADIUS, [center, p1, p2], 0)
...
while running:
    ...
    for ball in balls:
        if ball.pos[1] > HEIGHT or ball.pos[0] > WIDTH or ball.pos[1] < 0:
            balls.remove(ball)
            if len(balls) <= 1500 and increase:
        ...
        if (dist + BALL_RADIUS > CIRCLE_RADIUS):
            ...
            if ball.is_in:
                ...
                ball.pos = d_unit * \
                    (CIRCLE_RADIUS - BALL_RADIUS) + CIRCLE_CENTER
    ...
    if len(balls) == 10 and (not increase):
        increase = True
